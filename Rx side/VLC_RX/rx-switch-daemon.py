...
def runBufferThreadUL(ul_buffer):
	global w_ulb
	w_ulb = pydivert.WinDivert('false', priority=-900, layer=pydivert.Layer(1))
	w_ulb.open()

	t_tmp = time()

	while True:
		if ul_buffer.qsize() > DELAY_NUM or time()-t_tmp > DELAY_TIME:
			t_tmp = time()
			while not ul_buffer.empty():
				p = ul_buffer.get_nowait()
				w_ulb.send(p, recalculate_checksum=False)
				pass
			pass
		pass
	pass

def runProxyThreadUL(ul_buffer):
	...
	while True:
		...
		if tmp:
			...
			if (p.tcp and p.tcp.ack) and ((not p.tcp.syn) and (not p.tcp.fin) and (not p.tcp.rst)):
				ul_buffer.put_nowait(p)
			else:
				w_ul.send(p, recalculate_checksum=False)
				pass
			pass
		pass
	pass
...
def init():
	...
	data_q = Queue.Queue()
	ul_buffer = Queue.Queue()

	bufferHandleUL = threading.Thread(target=runBufferThreadUL, args=(ul_buffer, ))
	proxyHandleUL = threading.Thread(target=runProxyThreadUL, args=(ul_buffer, ))
	proxyHandleDL = threading.Thread(target=runProxyThreadDL, args=(data_q, ))
	updateHandle = RxRegisterDaemon(proxy_map)
	exec_watch(bufferHandleUL, fatal=False, hook=rx_exit)
	exec_watch(proxyHandleUL, fatal=False, hook=rx_exit)
